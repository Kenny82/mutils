...
class VisEig:
  ...
  >>> vi.vis()
  """
  def __init__(self, N=63, fishEye=True, extent = 2):
    self.N = N
  ...
  def add( self, ev ):
    ...
    H = histogram2d( y,x,self.N, range=[[-self.extent,self.extent],[-self.extent,self.extent]] )[0]
    if any(y==0):
      Hr = histogram( x[y==0], bins=self.N, range=(-self.extent,self.extent) )[0]
      self.Hr = self.Hr + Hr
    self.H = self.H + H
  ...
  def vis( self, rlabels = [0.5,1], N=None):
    ...
    se = self.extent
    h = [imshow(log(1+self.H),interpolation='nearest',extent=[-se,se,-se,se])]
  ...
  def vis1( self, rlabels = [0.5,1], N=8, **kw ):
    ...
    pdict =  dict(color=[0,0,0],linestyle='--')
    pdict.update(**kw)
    self._decorate( rlabels, pdict )
    fig.set_visible(vis)    
    return h
    
  def _decorate( self, rlabels, lt,  ):
    """
    adds circles in the complex plane to indicate the magnitude of a complex 
    number
    rlabels: array of radii
    lt: line formatting dictionary
    """
    axis('equal')
...
def qqplot( Sy, Sx = gaussian.isf, **kwargs):
...
def qqplot2( Sy, Sx = gaussian.isf, **kwargs):
    """Quantile-quantile plot the samples against each other
       If Sx is a function, it is assumed to be the 
       inverse survival function to be used as a reference

       Update Version 2: Sy can be callable.
       In that case, it is considered as an inverse survival function.

       For example, to compare the Gaussian with Student's t distribution with
       dof=3, do:
       qqplot2(labmda t: scipy.stats.t.isf(t, 3)) """
    if callable(Sy):
        n = 50
        Sy = -Sy(linspace(.1 / n, 1 - .1 / n, n))
    Sy = asarray(Sy).flatten()
    Sy.sort()
    n = len(Sy)
    if callable(Sx):
