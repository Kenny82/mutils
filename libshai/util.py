...
class VisEig:
  ...
  def vis( self, rlabels = [0.5,1], N=None):
    ...
    fp = kw.get('fontproperties') # defaults to none
    self._decorate( rlabels, dict(color=[1,1,1],linestyle='--'), fp )
    fig.set_visible(vis)    
    return h
  ...
  def vis1( self, rlabels = [0.5,1], N=8, **kw ):
    ...
    fp = kw.get('fontproperties') # defaults to none
    self._decorate( rlabels, dict(color=[0,0,0],linestyle='--'), fp )
    fig.set_visible(vis)    
    return h
    
  def _decorate( self, rlabels, lt, fp=None ):
    axis('equal')
    ...
    for r in rlabels:
      ...
      if fp is not None:
        self.h_lbl.append(
                text(x,y,"%g" % r, color=lt['color'], fontproperties=fp)
        )
      else: 
        self.h_lbl.append(
...
def qqplot( Sy, Sx = gaussian.isf, **kwargs ):
    ...
    if 'marker' not in kwargs.keys():
        kwargs['marker'] = '.'
    if 'ls' not in kwargs.keys() or 'linestyle' not in kwargs.keys():
        kwargs['ls'] = ''
